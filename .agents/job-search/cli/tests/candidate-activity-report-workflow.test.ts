...
const applications: ApplicationRepository = { async create(value) { return { ok: true, value } }, async save(value) { return { ok: true, value } }, async getById(id) { return id === application.id ? { ok: true, value: structuredClone(application) } : { ok: false, error: { code: "NOT_FOUND", message: "missing" } } }, async list() { return { ok: true, value: [structuredClone(application)] } }, async remove() { return { ok: true, value: undefined } } }
const associations: CandidateApplicationAssociationRepository = { async create(value) { return { ok: true, value } }, async getByApplicationId(id) { return id === association.applicationId ? { ok: true, value: structuredClone(association) } : { ok: false, error: { code: "NOT_FOUND", message: "missing" } } }, async listByCandidateId(id) { return id === association.candidateId ? { ok: true, value: [structuredClone(association)] } : { ok: true, value: [] } }, async deleteByApplicationId() { return { ok: true, value: undefined } } }
