...
import { deriveCandidateCareerActions, loadCandidateAnalytics, loadCandidateInterviewPracticeAnalytics, loadCandidateRequirementInsights, parseAnalyticsPeriod, type RequirementInsight, type SkillGapInsight } from "@/lib/analytics";
...
const VISIBLE_INSIGHT_ROWS = 5;

function splitRows<T>(items: T[]): { visible: T[]; rest: T[] } {
  return { visible: items.slice(0, VISIBLE_INSIGHT_ROWS), rest: items.slice(VISIBLE_INSIGHT_ROWS) };
}

function requirementRow(item: RequirementInsight) {
  return <article className={styles.candidateRow} key={item.requirementId}><strong>{item.label}</strong><span>{item.matched} · {item.missing} · {item.conflicting} · {item.unknown}</span></article>;
}

function skillGapRow(gap: SkillGapInsight) {
  return <article className={styles.candidateRow} key={gap.gapId}><strong>{gap.label}</strong><span>Förekom i {gap.applicationsRepresented} ansökningar · {gap.occurrences} förekomster</span></article>;
}
        ...
        {!result.configured ? (
          ...
          <>
            ...
            <section className={styles.panel} aria-labelledby="requirements-heading">
              ...
              {!requirementInsights.configured ? <div className={styles.emptyState}><strong>Analysarkiven är inte konfigurerade</strong><p>Krav och kompetens kan inte visas ännu.</p></div> : requirementInsights.error ? <div className={styles.emptyState}><strong>Kravanalysen kunde inte läsas</strong><p>Kontrollera analysarkiven och försök igen.</p></div> : requirementInsights.insights && requirementInsights.insights.applicationsTotal === 0 ? <div className={styles.emptyState}><strong>Inga ansökningar finns ännu</strong><p>Sparade matchningsanalyser visas när ansökningar finns.</p></div> : requirementInsights.insights && requirementInsights.insights.applicationsWithAnalysis === 0 ? <div className={styles.emptyState}><strong>Inga sparade matchningsanalyser</strong><p>Ansökningarna saknar beständiga analyser.</p></div> : requirementInsights.insights ? <><p className={styles.subtitle}>Visar {requirementInsights.insights.applicationsWithAnalysis} av {requirementInsights.insights.applicationsTotal} ansökningar med sparad analys. Detta är historiska fakta, inte en bedömning av kandidaten.</p>{requirementInsights.insights.requirements.length > 0 ? (() => { const { visible, rest } = splitRows(requirementInsights.insights!.requirements); return <div className={styles.candidateList}><article className={styles.candidateRow}><strong>Krav</strong><span>Matchad · Saknas · Konflikt · Okänd</span></article>{visible.map(requirementRow)}{rest.length > 0 && <details className={styles.analysisDetails}><summary>Visa alla krav ({requirementInsights.insights!.requirements.length})</summary><div className={styles.candidateList}>{rest.map(requirementRow)}</div></details>}</div>; })() : <p>Inga krav med sparad matchningsinformation hittades.</p>}{requirementInsights.insights.skillGaps.length > 0 && (() => { const { visible, rest } = splitRows(requirementInsights.insights!.skillGaps); return <><h3>Kompetensgap i sparade analyser</h3><div className={styles.candidateList}>{visible.map(skillGapRow)}</div>{rest.length > 0 && <details className={styles.analysisDetails}><summary>Visa alla kompetensgap ({requirementInsights.insights!.skillGaps.length})</summary><div className={styles.candidateList}>{rest.map(skillGapRow)}</div></details>}</>; })()}<h3>Förslag på nästa steg</h3><CareerActionList actions={deriveCandidateCareerActions(requirementInsights.insights)} /></> : null}
            </section>
            <section className={styles.panel}>
              <div className={styles.panelHeader}>
                <div>
