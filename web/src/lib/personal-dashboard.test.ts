...
const profile = { headline: "Support", skills: { technical: [], soft: [] }, workExperience: [{ title: "Support", company: "Test", location: "Stockholm" }], education: [{ degree: "Examen", field: "IT", institution: "Testskolan" }] } as never;
const baseCv = { headline: "Support", workExperience: [{}], education: [{}] } as never;
...
describe("personal dashboard model", () => {
  ...
  it("suggests jobs when the profile and Base CV are ready but applications are empty", () => {
    const model = buildPersonalDashboardModel("candidate-a", { profile, baseCv, applications: [] });
  ...
  it("offers interview continuation only for an application with interview status", () => {
    const model = buildPersonalDashboardModel("candidate-a", { profile, baseCv, applications: [application("app-interview", "interview", "2026-01-01T00:00:00.000Z")] });
